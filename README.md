...
- **Refresh** just rereads what's already been collected — no new fetch. **Fresh check** actually checks everything again, Edpuzzle included by default, which takes about a minute and opens a browser window for it. A square **Stop** button (placeholder icon for now) appears next to it for as long as a check is actually running, and ends it. If your teachers already post Edpuzzle assignments through Google Classroom too, there's a setting to skip Edpuzzle entirely — a fresh check is then faster and never opens that window.
