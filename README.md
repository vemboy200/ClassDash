...
- Three small dots next to the gear icon, one per platform — blue means the last check for that platform actually worked, red means it failed, gray means it isn't being checked at all (Canvas with no address set, Edpuzzle turned off, or no check has run yet). Click them for the full breakdown, timestamps and error detail included.
